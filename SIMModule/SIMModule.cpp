...
String SIMModule::listMessages(const String &type) {
    sendAT("AT+CMGF=1");
    return sendATWithResponse("AT+CMGL=\"" + type + "\"");
}

void SIMModule::enableCallerID() { sendAT("AT+CLIP=1"); }
void SIMModule::beginGetSignalStrength() {
    signalBuffer = "";
    signalState = WAITING_RESPONSE;
    signalStartTime = millis();
    _serial.println("AT+CSQ");
}
String SIMModule::pollGetSignalStrength() {
    if (signalState == WAITING_RESPONSE) {
        while (_serial.available()) {
            char c = _serial.read();
            signalBuffer += c;
        }

        // End detection: either timeout or found expected response
        if (millis() - signalStartTime > 1000 || signalBuffer.indexOf("+CSQ:") >= 0) {
            signalState = IDLE;

            int idx = signalBuffer.indexOf("+CSQ:");
            if (idx >= 0) {
                int end = signalBuffer.indexOf('\n', idx);
                if (end > idx) {
                    String str = signalBuffer.substring(idx, end);
                    str.trim();
                    return str;
                } else {
                    String str = signalBuffer.substring(idx);
                    str.trim();
                    return str;
                }
            }

            return "ERROR"; // response malformed
        }
    }
...
// Misc
String SIMModule::getInfo() { return sendATWithResponse("ATI"); }
String SIMModule::getFirmwareVersion() { return sendATWithResponse("AT+GMR"); }
String SIMModule::getICCID() { return sendATWithResponse("AT+CCID"); }
String SIMModule::showIMEI() { return sendATWithResponse("AT+CSGN"); }
